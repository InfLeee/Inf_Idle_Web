import { EQUIPMENT_SLOTS, ITEM_CATEGORY, MAP_LEVEL_MODE, RARITY_META, itemizationCatalog, resolveMonsterLevel } from "../../packages/itemization-core/src/index.js";
import { createAuthoritativeEquipmentService } from "../../packages/server-core/src/authoritative-equipment-service.js";
import { acceptIdentifiedSkillCardGrant, characterProgressionAuthority, commitCharacterProgression, currentLoadoutSnapshot } from "./loadout-authority.js?v=mastery-stats-2";
...
const SAVE_KEY = "inf-idle.m4b-backpack.v0";
...
if ($("m4ItemizationLab")) {
  ...
  function syncCharacter(snapshot) { const progression = characterProgressionAuthority.setAuthorityState({ bonuses: { derived: snapshot.bonuses.derived, provenance: snapshot.bonuses.provenance } }); const loadout = commitCharacterProgression(progression); persist(snapshot); $("m4BuildProof").textContent = loadout.compiledBuild ? `已实时重编译 · ${loadout.compiledBuild.buildHash.slice(0, 12)}` : "属性已同步 · 当前未装备可战斗武器"; }
  ...
  function itemDetailHtml(item, options = {}) {
    const affixes = (item.affixes ?? []).map((affix) => `<li><span>${affix.kind === "prefix" ? "前" : "后"} · T${affix.tier} ${affix.name}</span><b>+${affix.value} ${statLabel(affix.statId)}</b><small>${affix.minimum}–${affix.maximum}</small></li>`).join("");
    ...
    const discardAction = options.action && !options.equipped ? `<button id="m4DetailDiscard" class="danger" type="button">丢弃（测试整理）</button>` : "";
    const weaponMeta = item.category === ITEM_CATEGORY.WEAPON ? `<div class="m4-weapon-rolls"><b>${item.skillCardSocketCount} 个技能卡孔</b><b>每技能固定 ${item.supportSocketsPerSkill} 个辅助孔</b><span>赠送技能：${item.grantedSocketedSkillCard ? `${item.grantedSocketedSkillCard.name} Lv.${item.grantedSocketedSkillCard.skillLevel}（可拆）` : "无"}</span><span>武器技能 ${item.rolledWeaponSkills.length}/5：${item.rolledWeaponSkills.join("、")}</span></div>` : "";
    return `<header><span>${item.icon}</span><div><small>${options.title ?? "选中物品"} · ${detail} · ${rarityLabel(item)} · 物品 Lv.${item.itemLevel}</small><strong>${item.name}</strong></div></header>${weaponMeta}${affixes ? `<ul>${affixes}</ul>` : `<p class="m4-no-affix">普通物品 · 无随机词缀</p>`}<footer><span>需求等级 ${item.requiredLevel}</span><div>${equipAction}${discardAction}</div></footer>`;
  }
  ...
  function renderBag(snapshot) {
    ...
    if (bagFilter === "equipment") {
      ...
      if (selected) {
        ...
        $("m4CompareDetail").innerHTML = equipped && equipped.instanceId !== selected.instanceId ? itemDetailHtml(equipped, { title: "当前已穿戴" }) : `<div class="m4-detail-empty"><b>${selected.category === ITEM_CATEGORY.EQUIPMENT ? "该部位当前为空" : "武器构筑入口将在 M4C 接通"}</b><span>${selected.category === ITEM_CATEGORY.EQUIPMENT ? "穿戴后这里显示替换前装备" : "当前已完整保留孔位、赠送技能及武器技能 Roll"}</span></div>`;
  ...
  updateSubtypeOptions(); renderPickupLog(); $("m4BuildProof").textContent = currentLoadoutSnapshot().compiledBuild ? "等待装备变化 · 当前战斗构筑正常" : "当前未装备可战斗武器"; render();
}
